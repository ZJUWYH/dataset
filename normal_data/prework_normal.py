...
input_file = r'E:\Course Assessment\机电\dataset\normal_data\result_normal.csv'
...
for i in csv_list:  # 循环读取同文件夹下的csv文件
    fr = open(i, 'rb').read()
    with open('result_normal.csv', 'ab') as f:  # 将结果保存为result.csv
        f.write(fr)
print(csv_list)
...
for i in range(sam_num):
    sam_list[sam_quantity*i: sam_quantity*(i+1)] = i+1
df = pd.DataFrame({'': sam_list})
